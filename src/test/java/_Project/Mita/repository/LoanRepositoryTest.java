...
class LoanRepositoryTest {
    ...
    void 未返却かつ期日超過のデータのみが期日の昇順で取得できること() {
        ...
        Loan loan1 = createLoan(book, user, targetDate, targetDate.minusDays(1), null);                      // 期待するデータ①：未返却（Null）かつ 期日超過（前日）
        Loan loan2 = createLoan(book, user, targetDate, targetDate.minusDays(2), null);                      // 期待するデータ②：未返却（Null）かつ 期日超過（2日前） -> loan1より古いため、こちらが1番目に取得されるべき
        Loan loan3 = createLoan(book, user, targetDate, targetDate.minusDays(1), targetDate.minusDays(1));   // 除外データ①：返却済（NotNull）かつ 期日超過
        Loan loan4 = createLoan(book, user, targetDate, targetDate, null);                                  // 除外データ②：未返却（Null）かつ 期日当日（Beforeに含まれない）
        Loan loan5 = createLoan(book, user, targetDate, targetDate.plusDays(1), targetDate.plusDays(1));     // 除外データ③：返却済（NotNull）かつ 期日前
    ...
    void 指定したユーザーの貸出履歴のみが貸出日の降順で取得できること() {
        ...
        Loan loan1 = createLoan(book, userA, baseDate, baseDate.plusDays(7), null);

        // 期待するデータ②：ユーザーA、貸出日（古い：前日）
        Loan loan2 = createLoan(book, userA, baseDate.minusDays(1), baseDate.plusDays(6), null);

        // 除外データ：ユーザーBの貸出データ
        Loan loan3 = createLoan(book, userB, baseDate, baseDate.plusDays(7), null);
    ...
    void 指定した本とユーザーの未返却データが取得できること() {
        ...
        // 除外データ①：本A × ユーザーA × 返却済
        Loan loan2 = createLoan(bookA, userA, baseDate, baseDate.plusDays(7), baseDate.plusDays(1));

        // 除外データ②：本B(違う本) × ユーザーA × 未返却
        Loan loan3 = createLoan(bookB, userA, baseDate, baseDate.plusDays(7), null);

        // 除外データ③：本A × ユーザーB(違う人) × 未返却
        Loan loan4 = createLoan(bookA, userB, baseDate, baseDate.plusDays(7), null);
        ...
        Optional<Loan> result = loanRepository.findByBook_BookIdAndUser_UserIdAndReturnDateIsNull(bookA.getBookId(), userA.getUserId());
    ...
    void すべての貸出データが貸出日の降順で取得できること() {
        ...
        // 貸出日：昨日 (降順で2番目)
        Loan loan1 = createLoan(book, user, baseDate.minusDays(1), baseDate.plusDays(6), null);

        // 貸出日：今日（一番新しい ➔ 降順で1番目）
        Loan loan2 = createLoan(book, user, baseDate, baseDate.plusDays(7), null);

        // 貸出日：一昨日（一番古い ➔ 降順で3番目）
        Loan loan3 = createLoan(book, user, baseDate.minusDays(2), baseDate.plusDays(5), null);
