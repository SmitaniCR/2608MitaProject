...
public class LoanRequestTest {
    ...
    private Validator validator;//Mockと発想は違うが、「道具を用意する」という点では同じ
    ...
    void success_whenBookIdIsPresent() {
        LoanRequest loanRequest = new LoanRequest(1L);
    ...
    void error_whenBookIdIsNull() {
        LoanRequest loanRequest = new LoanRequest(null);
        Set<ConstraintViolation<LoanRequest>> violations = validator.validate(loanRequest);
        assertThat(violations).hasSize(1);
        ConstraintViolation<LoanRequest> violation = violations.iterator().next();
